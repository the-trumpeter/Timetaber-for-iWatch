	...
	var body: some View {
		Menu("Export/Import...") {
			...
			Button("Export Timetable...", systemImage: "square.and.arrow.up") {
				Logger.files.notice("Exporting timetable...")
				do {
					Logger.files.notice("└─ Compiling JSON...")
					exportData = JsonDocument(
						json:
		...
		.alert("Importing failed", isPresented: Binding(get: {importFailed != nil},set:{_ in} ) ) {
			...
			switch importFailed {
				...
				default: Text("Something went wrong with this alert, as well as your import.\nError \(#line)")
			}
		}

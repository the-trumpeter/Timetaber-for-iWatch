...
class Storage: ObservableObject {
	...
	//Multiple timetables: current/active timetable
	...
	func applyChanges(_ changes: [Change] ) {
		for change in changes {
			...
			case .times_variants_delete(let del, timetable: let tblIndex):
				...
				// Replace any mapping entries referencing the deleted variant with .standard
				for (wkday, mapping) in timetable.times.mapping {
					if case .variant(let key) = mapping, key == del {
						timetable.times.mapping[wkday] = .standard
						Logger.timetableChanges.notice("Replaced deleted variant in mapping for day \(wkday) with .standard")
						// Remap periods for this day to standard
						let oldVariant: [UUID: Times.Period] = {
							switch mapping {
								case .standard: return timetable.times.standard
								case .variant(let key): return timetable.times.variants[key]?.variant ?? [:]
						...
						let oldPeriodsByName = Dictionary(uniqueKeysWithValues: oldVariant.map { ($0.value.name, $0.key) })
						let newVariant = timetable.times.standard
						for weekIdx in timetable.timetable.indices {
							var week = timetable.timetable[weekIdx]
							let oldAssignments: [UUID: Times.Period.Contents]? = switch wkday {
								case 2: week.monday
...
//MARK: Previews
extension Storage {
	   static let preview: Storage = {
		   let store = Storage()
		   store.timetables = [chaos]
		   store.ActiveTimetable = 0
		   return store
	   }()
   }
